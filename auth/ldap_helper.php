...
//    $ns_callback = _NAMESPACE_ . "\\$callback";
    ...
    foreach ($config_items as $item)
    {
        ...
        // The case where the config item isn't an array is handled
        // further down
    }
        ...
        if ($ldap)
        {
            ...
            if (isset($all_ldap_opts['ldap_dn_search_attrib'][$idx]))
            {
                ...
                else
                {
                    ...
                    authLdapDebug("1authLdapAction: base_dn '".
                        $all_ldap_opts['ldap_base_dn'][$idx].
                        "' user '$user' dn '$dn'");
                }
            }
...
function authLdapGetDetails($user)
{
    $name = '';
    $object = array();

    $res = authLdapAction("authLdapGetDetailsCallback", $user, $object);
...
function authLdapGetDetailsCallback(&$ldap, $base_dn, $dn, $user_search,
                                 $user, &$object)
{
    $name_attrib = $object['config']['ldap_name_attrib'];
    ...
    if ($ldap && $base_dn && $dn && $user_search)
    {
        $res = @ldap_read(
            ...
            $dn,
            "(objectclass=*)"
            ,array('description','businesscategory','cn','mail','samaccountname')
        );
        if (@ldap_count_entries($ldap, $res) > 0)
        {
            authLdapDebug("authLdapGetNameCallback: search successful");
            ...
            $object['details'] = [];
            $object['details']['nome'] = $entries[0]['description'][0];
            $object['details']['categoria'] = $entries[0]['businesscategory'][0];
            $object['details']['id'] = $entries[0]['cn'][0];
            $object['details']['email'] = $entries[0]['mail'][0];
            $object['details']['usuario'] = $entries[0]['samaccountname'][0];
