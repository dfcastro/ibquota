 ...
 * Lista Usuários Administrativos e Adição Rápida
 */

// 1. INCLUDES CORRIGIDOS: Recuando dois níveis para aceder à pasta core
...
<div class="d-flex justify-content-between align-items-center mb-4 mt-2 border-bottom border-light pb-3">
  <div>
    ...
    <p class="text-muted mb-0 small">Controle quem tem acesso ao painel do IFQUOTA</p>
  </div>
  <a href="<?php echo $BASE_URL; ?>/admin/dashboard" class="btn btn-outline-secondary shadow-sm"><i class="bi bi-arrow-left me-1"></i> Voltar ao Dashboard</a>
</div>
...
if (isset($_GET['msg'])) {
  ...
  if (array_key_exists($_GET['msg'], $mensagens)) {
    echo "<div class='alert alert-{$tipo} alert-dismissible fade show shadow-sm border-0 mb-4' role='alert'>
              ...
              <?php while ($stmt->fetch()) { ?>
                <tr>
                  <td class="ps-4">
                    ...
                    if ($permissao == 3) echo '<span class="badge bg-danger rounded-pill shadow-sm"><i class="bi bi-star-fill me-1"></i>Diretor(a)</span>';
                    elseif ($permissao == 2) echo '<span class="badge bg-primary rounded-pill shadow-sm">Admin Geral</span>';
                    elseif ($permissao == 1) echo '<span class="badge bg-success rounded-pill shadow-sm">Admin Impressão</span>';
                    else echo '<span class="badge bg-secondary rounded-pill shadow-sm">Visualiza Relatórios</span>';
                    ?>
                  </td>
                  <td>
    ...
    <div class="card shadow-sm border-0 border-top border-success border-3">
      ...
      <div class="card-body bg-light">
        ...
        <form action="<?php echo $BASE_URL; ?>/admin/usuarios/add" method="post">
          ...
          <button type="submit" class="btn btn-success w-100 fw-bold shadow-sm"><i class="bi bi-check-circle me-2"></i>Cadastrar Usuário</button>
        </form>
      </div>
    </div>
  </div>
...
<?php
// 3. FOOTER CORRIGIDO: Recuando dois níveis
