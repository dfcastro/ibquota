...
if (isset($_GET['remove_grupo'])) {
    ...
    $stmt_u->execute();
    $stmt_u->bind_result($nome_usuario_rm);
    $stmt_u->fetch();
    $stmt_u->close();
...
if (isset($_POST['acao']) && $_POST['acao'] == 'add_quota') {
    ...
    if (strlen($grupo) < 1) {
        ...
        $chk_q = $mysqli->prepare("SELECT cod_quota_usuario FROM quota_usuario WHERE cod_politica = ? AND usuario = ? AND grupo = ?");
        ...
        if ($chk_q->num_rows < 1) {
            ...
            $upd_q = $mysqli->prepare("UPDATE quota_usuario SET quota = quota + ? WHERE cod_politica = ? AND usuario = ? AND grupo = ?");
            $upd_q->bind_param('iiss', $quota_adicional, $cod_politica, $usuario, $grupo);
            $upd_q->execute();
        }
